...
class DataTable(BoxLayout):
    def __init__(self,table='', **kwargs):
        ...
        table_data = []
        for i in col_titles:
            table_data.append({'text':str(i),'size_hint_y':None,'height':30, 'bcolor':(.6,.45,.45,1)})
        
        for r in range(rows_len):
            for t in col_titles:
                table_data.append({'text':str(products[t][r]),'size_hint_y':None,'height':30,'bcolor':(.6,.25,.25,1)})
