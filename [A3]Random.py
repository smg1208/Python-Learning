class RandomPowerBall():
    ...
    def eachTryingTime(self):
        ...
        while len(result) < self.getTypeofPowerBall()[0]:
            ...
            resultRandom = [listRandom.count(x) for x in range(
                1, self.getTypeofPowerBall()[1]+1)]
            if resultRandom.count(max(resultRandom)) == 1:
                if resultRandom.index(max(resultRandom))+1 not in result:
                    result.append(resultRandom.index(max(resultRandom))+1)
        return result
